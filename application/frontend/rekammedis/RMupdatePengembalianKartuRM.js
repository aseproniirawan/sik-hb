...
function RMupdatePengembalianKartuRM(){
	var pageSize = 50;
	...
	var ds_pengembalian = new Ext.data.JsonStore({		
		proxy: new Ext.data.HttpProxy({
			url: BASE_URL + 'rekammedis_controller/getPengeluaranKartuRM',
			method: 'POST'
		}),
		params: {
			start: 0, limit: 50
		...
		fields: [{
			...
			name: "tglminta", mapping: "tglkeluar"
		},{
			name: "jamminta", mapping: "jamkeluar"
		},{
			name: "idbagian", mapping: "idbagian"
		},{
			name: "nmbagian", mapping: "nmbagian"
	...
	var grid_nya = new Ext.grid.GridPanel({
		id: 'gPengembalianKartuRM', sm: cbGrid, store: ds_pengembalian, forceFit: true,		
		...
		},
		{
			...
			header: 'Tanggal<br>Terima',	width: 75, dataIndex: 'tglminta',
			align: 'center', sortable: true
		},{
			header: 'Jam<br>Keluar', width: 75, dataIndex: 'jamminta',
			align: 'center', sortable: true
		},{
			header: 'Unit/Ruangan<br>Keluar', width: 140, dataIndex: 'nmbagian',
			align: 'left', sortable: true
		},{
	...
	var main_form = new Ext.form.FormPanel({		
		...
		tbar: [{
			...
			text: 'Terima', iconCls: 'silk-accept', handler: function(){
				terimaRM();
			}
		}],
		items: [{
			xtype: 'fieldset',
			title: 'Daftar Penerimaan Berkas RM',
			items: [{
					xtype: 'container',
					style: 'padding: 5px',
					layout: 'column',
					...
					items:[{
						...
						items: [{
							...
							items: [{
								...
								margins: {top:3, right:0, bottom:10, left:0}
							},{
	...
	function cariPenerimaan(){
		ds_pengembalian.setBaseParam('nmjnspelayanan',Ext.getCmp('cbPelayanan').getValue());
		ds_pengembalian.setBaseParam('nmstpasien',Ext.getCmp('cbStatusPasien').getValue());
		ds_pengembalian.setBaseParam('nmpasien',Ext.getCmp('tNamaPasien').getValue());
		ds_pengembalian.setBaseParam('norm',Ext.getCmp('tNoRM').getValue());
		ds_pengembalian.setBaseParam('nmbagian',Ext.getCmp('tUnitRuang').getValue());
		ds_pengembalian.setBaseParam('kdlokasi',Ext.getCmp('cmbLokasi').getValue());
		ds_pengembalian.reload();
	}
	
	function terimaRM(){
		Ext.Ajax.request({
			url: BASE_URL + 'rekammedis_controller/terimaRM',
			params: {
				arr : Ext.encode(arr)
			}, 
			success: function(response){
				Ext.getCmp("tPesan").setText('Berkas RM berhasil diterima');
				ds_pengembalian.reload();
			},
			failure: function(response){
				Ext.getCmp('tPesan').setText('Berkas RM gagal diterima');
			}
		});
	}
	...
	function terimaRMScann(){
		Ext.Ajax.request({
			url: BASE_URL + 'rekammedis_controller/terimaRMScann',
			params: {
