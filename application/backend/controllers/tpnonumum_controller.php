...
class Tpnonumum_Controller extends Controller {
	...
	function get_tpnonumum(){
		...
		//$this->db->where('tarif.idpenjamin > 1');
		//$this->db->or_where('tarif.idpenjamin', 3);
		$this->db->order_by('tarif.kdpelayanan');        
        
		$where = array();
        $where['tarif.idpenjamin']= $_POST['idpenjamin'];      
	...
	function update1field_tpnonumum(){ 				
		$field = $_POST['field'];
		$value = $_POST['value'];
    ...
    function update(){  
		...
		$where['idpenjamin']=$this->input->post('idpenjamin');
	...
	function getFieldsAndValues(){
		$idpenjamin = (isset($_POST['idpenjamin']))? $_POST['idpenjamin'] : 2;
	...
	function cekkdpelayanan(){
        $q = "SELECT count(*) as kdpelayanan FROM tarif where kdpelayanan='".$_POST['kdpelayanan']."' AND idklstarif='".$_POST['idklstarif']."' AND idpenjamin= '".$_POST['idpenjamin']."'" ;
